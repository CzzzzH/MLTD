...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, sample_method=None, block_type="down"):
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(dim=dim, num_heads=num_heads, block_type=block_type,
                ffn_expansion_factor=2.66, bias=False, LayerNorm_type="BiasFree") for i in range(depth)])
...
class RecurrentBlock(nn.Module):
    ...
    def __init__(self, dim, depth, num_heads, block_type, recon_dim):
        ...
        if block_type == "down":
            ...
            self.recon_conv = nn.Conv2d(dim, recon_dim + 4, 1)
        ...
        self.block_type = block_type
        self.transformer_layer = BasicLayer(dim=dim, depth=depth, num_heads=num_heads,
            sample_method=sample_method, block_type=block_type)
        self.kernels = None
...
class MLTD(nn.Module):
    
    def __init__(self, cfg):
        
        super(MLTD, self).__init__()
    ...
    def forward(self, x):
        ...
        denoised_coarse = [[[None] * 4 for j in range(4)] for i in range(t)]
        
        for i in range(t):
            input, motion, radiance_all = x[:, i, :22], x[:, i, 22:25], x[:, i, 25:]
            radiances = [radiance_all[:, k * 3:(k + 1) * 3] for k in range(4)]
            
            # If the animation is split into multiple clips, we should not reset the hidden states
            if i == 0 and self.reset_flag:
                self.recurrent_unet.reset_hidden(radiances[0])
                if hasattr(self.cfg, 'split') and self.cfg.split:
            ...
            for j in range(4):
                ...
                for k in range(4):
                    radiance_down = F.interpolate(radiances[k], scale_factor=0.5 ** (3 - j)) \
                        if j < 3 else radiances[k]
                    
                    kernels_temporal = self.recurrent_unet.up_layers[j].kernels[:, recon_offset:recon_offset+self.kernel_size_temporal] \
                        if j < 3 else result[:, recon_offset:recon_offset+self.kernel_size_temporal]
                    recon_offset += self.kernel_size_temporal
                    
                    kernels = self.recurrent_unet.up_layers[j].kernels[:, recon_offset:recon_offset+self.kernel_size] \
                        if j < 3 else result[:, recon_offset:recon_offset+self.kernel_size]
                    recon_offset += self.kernel_size
                    
                    alpha_0 = self.recurrent_unet.up_layers[j].kernels[:, recon_offset:recon_offset+1] \
                        if j < 3 else result[:, recon_offset:recon_offset+1] 
                    ...
                    if i == 0:
                        if hasattr(self.cfg, 'split') and self.cfg.split and self.denoised_coarse is not None:
                            denoised_coarse_warp = warp_tensor(self.denoised_coarse[j][k], motion)
                            denoised_coarse[i][j][k] = (1 - alpha_0) * self.apply_kernels(radiance_down.contiguous(), kernels) + \
                                                            alpha_0  * self.apply_kernels(denoised_coarse_warp, kernels_temporal)
                        else:
                            denoised_coarse[i][j][k] = self.apply_kernels(radiance_down.contiguous(), kernels)
                    else:
                        denoised_coarse_warp = warp_tensor(denoised_coarse[i - 1][j][k], motion)
                        denoised_coarse[i][j][k] = (1 - alpha_0) * self.apply_kernels(radiance_down.contiguous(), kernels) + \
                                                        alpha_0  * self.apply_kernels(denoised_coarse_warp, kernels_temporal)
            
            for k in range(4): 
                denoised_fine = denoised_coarse[i][0][k]

                for j in range(3):
                    recon_offset = (self.kernel_size_temporal + self.kernel_size + 1) * 4 + k
                    ...
                    denoised_down = F.interpolate(denoised_coarse[i][j + 1][k], scale_factor=0.5)
                    denoised_up = F.interpolate(alpha_1 * denoised_down, scale_factor=2)
                    denoised_fine = denoised_coarse[i][j + 1][k] - denoised_up + F.interpolate(alpha_1 * denoised_fine, scale_factor=2)
                preds[:, i] = preds[:, i] + denoised_fine
